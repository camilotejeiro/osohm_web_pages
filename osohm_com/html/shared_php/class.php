		...
		function register($redirect) 
		{
			...
			if ( empty ( $_POST ) == FALSE ) 
			{
				...
				if ( $referrer == $current ) 
				{
					...
					$insert = $osohm_database->insert($link, $table, $fields, $values);
		...
		function login($redirect) 
		{
			...
			if (empty( $_POST ) == FALSE) 
			{
				...
				if ( $subpass == $stopass ) 
				{
					...
					setcookie('osohmlogauth[user]', $subname, 0, '', '', '', true);
					setcookie('osohmlogauth[authID]', $authID, 0, '', '', '', true);
